...
import algorithm.Environment as Env
...
from utils.my_utils import dir_check, flatten, write_xlsx
...
actorpath='./output/'
dir_check(path)
actordict = actorpath + 'actorreal1.pth'#神经网络参数
if display_VRPTW:
    import VRPTW
# -----------------hyperparameters--------------------
...
def caluate_and_log_reward(g: Env.Graph, log_config: dict):
    ...
    postman_assigned_index, postman_assigned_timewindow, assignment_method, postman_assigned_staytime,assignment_time_order = g.get_assigned()
...
graph_seed = 0
num_graphs = 25# num graphs：how many graphs to be generated
...
try:
    ...
    while graph_seed < num_graphs: #num graphs：要生成多少个graph，graphseed是生成graph的随机种子
        ...
        for ep in allepisodes:
            ...
            g1 = Env.graph_generator(g1, NN, randomseed=graph_seed, graphname=None,Euclidean=Euclidean,Mag_Factor=Mag_Factor) #NN: number of nodes in the graph
            ...
            if display_myopic:
                ...
                episode_reward,_ ,_, _= caluate_and_log_reward(g2, log_cfg)
                ...
                postman_assigned_nodes, postman_assigned_time, postman_assigned_distance, postman_assigned_popuptime, \
                postman_assigned_index, postman_assigned_timewindow, assignment_method, postman_assigned_staytime, \
                assignment_time_order = g2.get_assigned()
                postman_delivered_nodes, postman_delivered_times, postman_delivered_index = g2.get_delivered()
                state2.reset()
                g2.reset(NN)  # reset the graph
                g2, state2 = Env.random_generator(g2, lb, hb, NN, PN, state2, randomseed=ep)
                
                #  print(postman_assigned_time)
                #   print(assignment_time_order)
                for i in range(PN):
                    if len(assignment_time_order[i]) != 0:
                        #   print('sort')
                        postman_assigned_distance[i]=sort_together([assignment_time_order[i], postman_assigned_distance[i]])[1]
                        postman_assigned_time[i]=sort_together([assignment_time_order[i], postman_assigned_time[i]])[1]
                        postman_delivered_times[i]=sort_together([assignment_time_order[i], postman_delivered_times[i]])[1]
                ...
                # -------------------------RL initialisation-------------------------------------
                Avg_Travel_Dist = sum([sum(postman_assigned_distance[i]) for i in range(PN)])
                Avg_Idle_Time1 = 0
                for i in range(PN):
                ...
                if Avg_Idle_Time1 < 0:
                    ...
                    print(assignment_time_order)
                    sys.exit()
                Greedy_Idle_Time_all.append(Avg_Idle_Time1)
                Greedy_Travel_Dist_all.append(Avg_Travel_Dist)
            ...
            while not state3.Done:
                ...
                greedy_counter += 1
            RLgreedyepisodereward ,_,averagelateness, variance= caluate_and_log_reward(g3, {'fout':"not_save"})
            ...
            postman_assigned_nodes, postman_assigned_time, postman_assigned_distance, postman_assigned_popuptime, \
            postman_assigned_index, postman_assigned_timewindow, assignment_method, postman_assigned_staytime, \
            assignment_time_order = g3.get_assigned()
            postman_delivered_nodes, postman_delivered_times, postman_delivered_index = g3.get_delivered()
            #  print(postman_assigned_time)
            #   print(assignment_time_order)
            for i in range(PN):
                if len(assignment_time_order[i]) != 0:
                    #   print('sort')
                    postman_assigned_distance[i] = \
                    sort_together([assignment_time_order[i], postman_assigned_distance[i]])[1]
                    postman_assigned_time[i] = sort_together([assignment_time_order[i], postman_assigned_time[i]])[1]
                    postman_delivered_times[i] = sort_together([assignment_time_order[i], postman_delivered_times[i]])[
                        1]

            #-----------------------------end of greedy RL rollout---------------------------------------
            # -------------------------RL initialisation-------------------------------------
            Avg_Travel_Dist = sum([sum(postman_assigned_distance[i]) for i in range(PN)])
            Avg_Idle_Time = sum([postman_assigned_time[i][0] for i in range(PN)]) + \
                            sum([postman_assigned_time[i][j + 1] - postman_delivered_times[i][j] for i in range(PN) for
                                 j in range(len(postman_assigned_time[i]) - 1)])
            if Avg_Idle_Time < 0:
                print(postman_assigned_time)
                print(assignment_time_order)
                sys.exit()
            computingtime.append(time.time()-starttime1)
            Avg_Idle_Time_all.append(Avg_Idle_Time)
            Avg_Travel_Dist_all.append(Avg_Travel_Dist)
        graph_seed += 1
...
if is_training:
    ...
    test_results=pd.DataFrame({'greedy':Greedy_test_rewards,'RL':RL_test_rewards,'lateness':alllateness,'var':allvar
                ,'Avg_Idle_Time':Avg_Idle_Time_all,'Avg_Travel_Dist':Avg_Travel_Dist_all,'computingtime':computingtime,
                'Greedy_Idle_Time_all':Greedy_Idle_Time_all,'Greedy_Travel_Dist_all':Greedy_Travel_Dist_all})
