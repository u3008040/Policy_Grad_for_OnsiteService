...
class ResidualGatedGCNModel(nn.Module):
    ...
    def __init__(self, is_training, PN, Mag_Factor):
        ...
        self.input_conversion=nn.Linear(self.node_dim+self.postman_dim+2+NN_lower-1,self.hidden_dim,bias=False)
        # Define GCN Layers
    ...
    def forward(self, all_adjacency_np, allOD_np, x_nodes_np, postman_np, if_assignment_required,
        ...
        for p_idx in range(PN_total):
            for l_idx in range(NN_total):
                ...
                x1_np[p_idx,l_idx]=np.concatenate((postman_np[0,p_idx],x_nodes_np[0,l_idx],distance,property,g.nearest3nodes[l_idx]))#g.ODlist[l_idx]))
        x1=self.relu(self.input_conversion(torch.cat((torch.from_numpy(x1_np).type(torch.float32).to(self.device)
        ...
        for j in range(batch_size):
            ...
            if 1 < PN_unassigned and 1 < NN_unassigned:  # recurse to find all solutions
                ...
                for i in action[j].keys():
                    if action[j][i] != None:
                       ...
                       # print('assign postman',postman_indices[i],'to',1)
                        ...
                        postman_np[j, postman_idx, 7] = (state.current_time - state.node_earliest_popuptime[location_idx] -
                                            state.node_earliest_timewindow[location_idx]) / 50 #destination time
                        ...
                        if x_nodes_np[j, location_idx, 2] < 0:# or x_nodes_np[j, location_idx, 2]>1:
                           # print(x_nodes_np[j, location_idx, 2])
                            print('error, mission number smaller than 0')# or greater than 1')
                            raise KeyboardInterrupt
                ...
                NNassignment, NNlogprobs, Entropy, prevemb, additional_penalty = ResidualGatedGCNModel.forward(self,
                    all_adjacency_np, allOD_np, x_nodes_np, postman_np, if_assignment_required,
